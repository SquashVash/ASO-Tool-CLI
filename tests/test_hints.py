...
def client(conn: sqlite3.Connection, fetcher) -> hints.HintsClient:
    return hints.HintsClient(fetcher, conn)
...
async def test_unknown_country_fails_before_any_request(conn: sqlite3.Connection) -> None:
    async with fast_fetcher() as fetcher:
        with pytest.raises(hints.UnknownStorefront):
            await client(conn, fetcher).suggest("can", "zz")
...
@respx.mock
async def test_suggest_sends_the_storefront_header(conn: sqlite3.Connection) -> None:
    ...
    async with fast_fetcher() as fetcher:
        await client(conn, fetcher).suggest("candlestick", "de")
...
@respx.mock
async def test_suggest_caches_by_prefix_and_storefront(conn: sqlite3.Connection) -> None:
    route = respx.get(HINTS_URL).mock(
        return_value=httpx.Response(200, text=REAL.read_text(encoding="utf-8"))
    )
    async with fast_fetcher() as fetcher:
...
@respx.mock
async def test_empty_suggestions_are_cached_as_a_real_answer(conn: sqlite3.Connection) -> None:
    route = respx.get(HINTS_URL).mock(
...
async def test_failed_fetch_propagates_and_caches_nothing(conn: sqlite3.Connection) -> None:
    ...
    async with fast_fetcher(retry_attempts=2) as fetcher:
        with pytest.raises(FetchError):
            await client(conn, fetcher).suggest("candlestick", "us")
...
@respx.mock
async def test_hints_and_itunes_share_one_rate_limiter(conn: sqlite3.Connection) -> None:
    ...
    async with fast_fetcher() as fetcher:
        await client(conn, fetcher).suggest("candlestick", "us")
        await ITunesClient(fetcher, conn).search("candlestick patterns", "us")
